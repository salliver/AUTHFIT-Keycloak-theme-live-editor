<#function themeAttr name fallback>
    <#-- Keycloak 22's RealmBean does not expose realm attributes. Keep the
         theme functional by resolving the configured theme.properties value. -->
    <#local value = (realmAttributes[name])!fallback>
...
<!DOCTYPE html>
<html class="${properties.kcHtmlClass!}"<#if realm.internationalizationEnabled>
  lang="${locale.currentLanguageTag}"
</#if>>
